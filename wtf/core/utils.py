...
def make_embed(group_info, date):
    return dict(
        ...
        url=f"https://www.roblox.com/groups/{group_info['id']}",
         fields=[
            dict(name="Group Info:", value=group_info["name"]),
            dict(name="MemberCount", value=group_info["memberCount"]),
            dict(name="Clothing Count", value={clothingcount(group_info["memberCount"])}),
            dict(name="Funds", value=f"R$ {group_info['funds']}" if group_info["funds"] is not None else "?")
        ],
      

       color=random.randint(10000, 50000),
      
        footer=dict(
            text="Roset Finder | Hosted by halotan",
            icon_url="https://media.discordapp.net/attachments/931673318828937306/934894425228861580/20220123_203618.gif"
        ),
     thumbnail=dict(url='https://media.discordapp.net/attachments/931673318828937306/934894425228861580/20220123_203618.gif')
    )
